...
## Статус обновления (2026-09-10) — ДЕПЛОЙ ВЫПОЛНЕН

- Сервер 38.180.212.31 теперь на **v1.6.6** (`dbccc1e`) + фича Subnet Address
  (ветка `feature/subnet-address`, `abd0522`). Деплой: код залит git archive
  (overlay, без --delete), venv обновлён, сервис перезапущен 2026-09-10 10:24 UTC.
- Проверено после деплоя: login 200, / 302, 0 ошибок в journal, ssh до
  серверов работает, awg2-контейнер Up 2 months, data.json: 2 сервера/2 юзера
  (uid-миграция апстрима при первом старте — норма).
- Откат: `/root/awp_backup_pre_1.6.6_2026-09-10.tar.gz` (на сервере, без venv)
  + архивы в `../backups/`.
- Константа CURRENT_VERSION в app.py = v1.6.5 (отстаёт от релиза v1.6.6 — баг апстрима).
- `fix/awg2-dynamic-subnet-start-script` и локальный пребазированный `f5b9053`
  можно удалить (присутствуют в форке).

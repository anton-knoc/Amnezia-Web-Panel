...
## Статус обновления (2026-09-10)

- Фича Subnet Address проверена (8/8 юнит-проверок, найден и исправлен баг:
  шлюз теперь первый usable-адрес подсети, а не сетевой адрес), закоммичена,
  перебазирована на origin/main → ветка `feature/subnet-address` (`abd0522`,
  push в fork). Все 264 теста апстрима прошли.
- Бэкапы прод-сервера в `../backups/`: код+data.json (2.4M), `/opt/amnezia`,
  ключи+clientsTable из контейнера amnezia-awg2 (контейнер хранит состояние
  ВНУТРИ себя, mounts только /lib/modules).
- Осталось: деплой на сервер (шаги 5–6, требует явного подтверждения).
- `fix/awg2-dynamic-subnet-start-script` можно удалить после деплоя.
